...
export function Greeting({ name, isMorning }) {
  return (
    <div>
      {isMorning ? (
        <p>좋은 아침입니다, {name}님!</p>
  ...
  render() {
    ...
    return (
      <div>
        <h3>{name}</h3>
        <p>나이: {userAge}</p>
        <button onClick={this.handleIncreaseAge}>한 살 더 먹기</button>
      </div>
    );
  }
}

// 문제 2 전용 렌더링 컴포넌트 (클래스형 UserCard 사용)
export class Problem2 extends React.Component {
  render() {
    return (
      <div>
        <h2>문제 2</h2>
        <UserCard name="김철수" age={25} />
        <UserCard name="박영희" age={30} />
      </div>
    );
  }
}
...
//   * 달콤함
//   * 비타민 C
export function ItemDetailList({ items }) {
  return (
    <div>
...
export function HighlightBox({ children }) {
  return (
    <span style={{ backgroundColor: "yellow", padding: "0 4px" }}>
      {children}
    </span>
  );
}
...
export function Problem4() {
  return (
    <div>
      ...
      <p>
        아래 텍스트에서 중요한 부분은{" "}
        <HighlightBox>노란색으로 표시</HighlightBox>됩니다.
      </p>
    </div>
  );
}
